...
const AddProductDialog = ({ onProductAdded, product, onProductUpdated }: AddProductDialogProps) => {
    ...
    return (
      <div className="p-4 text-center">
        <p className="text-red-600 mb-2">
          {!user ? 'Please log in to add products.' : `Only sellers can add products. Your account type is: ${user.userType}`}
        </p>
        <Button onClick={() => window.location.href = '/login'} className="bg-green-600 hover:bg-green-700">
          {!user ? 'Login' : 'Switch to Seller Account'}
        </Button>
      </div>
    );
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Add form fields (excluding section as it's not part of the product model)
      Object.entries(formData).forEach(([key, value]) => {
        if (key !== 'section') { // Don't send section to backend
      ...
      console.log('Form data being sent:', {
        ...
        priceNumeric: formData.price.replace(/[^\d]/g, ""),
        token: token ? 'Token present' : 'No token',
        user: user ? `User: ${user.name} (${user.userType})` : 'No user'
      });
    ...
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {product ? 'Update your product information below.' : 'Fill in the details to add a new product to your inventory.'}
          </DialogDescription>
        </DialogHeader>
