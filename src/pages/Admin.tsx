...
const Admin = () => {
  ...
  useEffect(() => {
    console.log('Admin: useEffect triggered', { isAuthenticated, userType: user?.userType, loading, error });
    
    if (!isAuthenticated || user?.userType !== 'admin') {
      console.log('Admin access denied:', { isAuthenticated, userType: user?.userType });
  ...
  if (isLoading) {
    console.log('Admin: Auth context is loading...');
    return (
  ...
  const fetchAdminData = async () => {
    try {
      ...
      const [statsRes, usersRes, cattleRes, productsRes, newsRes, ratingsRes] = await Promise.all([
      ...
      console.log('Admin API responses:', {
        ...
        cattle: cattleRes.status,
        products: productsRes.status,
        news: newsRes.status,
        ratings: ratingsRes.status
      });
      ...
      if (newsRes.ok) {
        ...
        console.error('News response error:', newsRes.status, await newsRes.text());
      }
  ...
  if (loading) {
    console.log('Admin: Component is in loading state');
    return (
      <div className="min-h-screen py-8">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
  ...
  if (error && (!isAuthenticated || user?.userType !== 'admin')) {
    console.log('Admin: Component has error:', error);
    return (
      <div className="min-h-screen py-8">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center py-12">
  ...
  console.log('Admin: Rendering main dashboard', { stats, users, cattle, products, news, ratings });
            ...
            {activeTab === 'news' && (
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {news.map((newsItem) => (
                      <tr key={newsItem._id} className="border-b hover:bg-gray-50">
                        <td className="py-2">{newsItem.title}</td>
                        <td className="py-2">{newsItem.category}</td>
                        <td className="py-2">{newsItem.author}</td>
                        <td className="py-2">
                          <Button
                            variant="destructive"
                            size="sm"
                            onClick={() => handleDelete('news', newsItem._id, newsItem.title)}
                            disabled={deleteLoading[newsItem._id]}
                          >
                            {deleteLoading[newsItem._id] ? (
                              'Deleting...'
                            ) : (
                              <>
                                <Trash2 className="h-4 w-4 mr-1" />
